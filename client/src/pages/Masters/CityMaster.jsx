...
const CityMaster = () => {
  ...
  const fetchCities = async () => {
    try {
      ...
      const citiesWithIds = result.data.map((city, index) => ({
        ...city,
        id: city._id ?? index,
      }));

      setRows(citiesWithIds);
    } catch (error) {
      setRows([]);
      dispatch(
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (result.success) {
        setRows((prev) => prev.filter((item) => item.id !== cityToDelete.id));
      ...
      <Grid container spacing={2} alignItems="center">
        <Grid size={{ xs: 12, md: "auto" }}>
          ...
          <Typography variant="body2" color="text.secondary">
            Manage and monitor all cities in the system
          </Typography>
        </Grid>
      </Grid>
        ...
        >
          ...
          <DialogContent>
            <Alert severity="warning" sx={{ mb: 2 }}>
              This action cannot be undone. The city will be permanently removed
              from the system.
            </Alert>
            <Typography>
