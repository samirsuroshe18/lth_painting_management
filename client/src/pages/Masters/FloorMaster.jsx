...
const FloorMaster = () => {
  ...
  const fetchFloors = async () => {
    try {
      ...
      const floorWithIds = result.data.map((floor, index) => ({
        ...floor,
        id: floor._id ?? index,
      }));

      setRows(floorWithIds);
    } catch (error) {
      setRows([]);
      dispatch(
  ...
  const applyFilters = () => {
    ...
    if (searchTerm.trim()) {
      ...
      filtered = filtered.filter((floor) => {
        ...
        return floorName.includes(searchLower);
      });
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editMode) {
        const result = await updateFloor(editFloorId, {
          name: floorName,
          status,
        });
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (result.success) {
        setRows((prev) => prev.filter((item) => item.id !== floorToDelete.id));
  ...
  const handleExcelUpload = async () => {
    ...
    try {
      ...
      const result = await addFloorsFromExcel(selectedFile);
      
      setUploadResults(result.data);
      
      // Refresh the data to show new floors
      await fetchFloors();
    ...
    <Container maxWidth="xl" sx={{ py: 3 }}>
      ...
      <Grid container spacing={2} alignItems="center">
        <Grid size={{ xs: 12, md: "auto" }}>
          ...
          <Typography variant="body2" color="text.secondary">
            Manage and monitor all floor in the system
          </Typography>
        </Grid>
      </Grid>
            ...
            <Box display="flex" flexDirection="column" gap={3} pt={1}>
              <Alert severity="info">
                <Typography variant="body2">
                  Upload an Excel file with floors. The file should have columns: <strong>name</strong> and <strong>status</strong> (boolean: true/false).
                </Typography>
              </Alert>
              ...
              {uploadResults && (
                <Box>
                  ...
                  <Alert severity="success" sx={{ mb: 2 }}>
                    Successfully processed {uploadResults.summary?.successfullyCreated || 0} floors
                  </Alert>
                  
                  {uploadResults.summary?.failed > 0 && (
                    <Alert severity="warning" sx={{ mb: 2 }}>
                      {uploadResults.summary.failed} floors failed to create
                    </Alert>
                  )}
      ...
      {deleteDialogOpen && (
        ...
        >
          ...
          <DialogContent>
            <Alert severity="warning" sx={{ mb: 2 }}>
              This action cannot be undone. The floor will be permanently
